...
class SeedAction
  def seed(trains_manager, rail_car_manager, routes_manager, stations_manager)
    ...
    5.times do |i|
      start_index = i * 2 + 1
      end_index = i * 2 + 3
      routes_manager.create(stations_manager.find_or_create("Station #{start_index}"),
                            stations_manager.find_or_create("Station #{end_index}"))
    end
